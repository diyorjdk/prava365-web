...
export default function HowItWorksSection() {
  ...
  return (
    <section id="how-it-works" className="py-20 lg:py-28 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-100/80 text-emerald-800 dark:bg-emerald-950/80 dark:text-emerald-300 text-xs font-bold uppercase tracking-wider mb-4">
            Qanday ishlaydi?
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-slate-950 dark:text-white tracking-tight mb-4">
            Guvohnoma olish uchun <span className="text-emerald-600 dark:text-emerald-400">3 ta oson qadam</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          {steps.map((item) => {
            ...
            return (
              <div
                key={item.step}
                className="relative bg-slate-50 dark:bg-slate-800/50 rounded-3xl p-8 border border-slate-200/90 dark:border-slate-700/70 hover:border-slate-300 dark:hover:border-slate-600 transition-all flex flex-col items-start"
              >
                {/* Step indicator */}
